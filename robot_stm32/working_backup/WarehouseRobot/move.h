...
#define speed_5 pwmConvertor(11)
#define speed_2	pwmConvertor(6)
...
void move_delay_forward(uint16_t* speed0, uint16_t* speed1) {
	delay_ms(300);
	GPIOA->ODR &= (GPIOA->ODR ^ 2);
	GPIOA->ODR &= (GPIOA->ODR ^ 4);
...
void turnLeftDelay(uint16_t* speed0, uint16_t* speed1) {
	GPIOA->ODR &= (GPIOA->ODR ^ 2);
	GPIOA->ODR |= 4;
...
void turnRightDelay(uint16_t* speed0, uint16_t* speed1) {
	GPIOA->ODR |= 2;
				GPIOA->ODR &= (GPIOA->ODR ^ 4);
...
void turnBackLeftDelay(uint16_t* speed0, uint16_t* speed1) {
	GPIOA->ODR &= (GPIOA->ODR ^ 2);
	GPIOA->ODR |= 4;
...
void turnBackRightDelay(uint16_t* speed0, uint16_t* speed1) {
	GPIOA->ODR |= 2;
	GPIOA->ODR &= (GPIOA->ODR ^ 4);
...
void opustit_kleshni() {
	GPIOB->ODR &= (GPIOB->ODR ^ 32);
	delay_ms(1000);
	char * done = DONE;
	print(done);
}

void shavtit_perednyu() {
	GPIOB->ODR |= 128;
	delay_ms(1000);
	char * done = DONE;
	print(done);
...
void brosit_perednyu() {
	GPIOB->ODR &= (GPIOB->ODR ^ 128);
	delay_ms(1000);
	char * done = DONE;
	print(done);
}
